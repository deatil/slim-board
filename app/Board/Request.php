...
class Request
{
    ...
    public static function ip($type = 0, $adv = true)
    {
        ...
        if ($adv) {
            if (static::serverParam('HTTP_X_FORWARDED_FOR')) {
                $arr = explode(',', static::serverParam('HTTP_X_FORWARDED_FOR'));
                ...
                $ip = trim(current($arr));
            } elseif (static::serverParam('HTTP_CLIENT_IP')) {
                $ip = static::serverParam('HTTP_CLIENT_IP');
            } elseif (static::serverParam('REMOTE_ADDR')) {
                $ip = static::serverParam('REMOTE_ADDR');
            }
        } elseif (static::serverParam('REMOTE_ADDR')) {
            $ip = static::serverParam('REMOTE_ADDR');
        }
